                ...
                <div x-data="{ open: false }" class="border-b border-slate-100 last:border-0">
                    ...
                    <div class="bg-[#F8FAFC] px-6 py-4 flex items-center justify-between hover:bg-slate-100 transition cursor-pointer" @click="open = !open">
                        ...
                        <div class="flex items-center gap-4" @click.stop>
                            ...
                            <!-- Tombol Hapus Kec -->
                            <form id="form-delete-kec-{{ $kecamatan->id }}" action="{{ route('kecamatan.destroy', $kecamatan->id) }}" method="POST" class="inline">
                                @csrf @method('DELETE')
                                <button type="button" 
                                        @click="$dispatch('open-confirm', { title: 'Hapus Kecamatan?', msg: 'Semua desa di bawah kecamatan ini akan ikut terhapus. Lanjutkan?', btnText: 'Hapus', btnColor: 'red', formId: 'form-delete-kec-{{ $kecamatan->id }}' })" 
                            ...
                            @forelse($kecamatan->desas as $desa)
                                <div class="px-6 py-3 border-b border-slate-50 flex items-center justify-between hover:bg-slate-50 transition">
                                    ...
                                    <div class="flex items-center gap-4">
                                        ...
                                        <!-- Tombol Hapus Desa -->
                                        <form id="form-delete-desa-{{ $desa->id }}" action="{{ route('desa.destroy', $desa->id) }}" method="POST" class="inline">
                                            @csrf @method('DELETE')
                                            <button type="button" 
                                                    @click="$dispatch('open-confirm', { title: 'Hapus Desa?', msg: 'Yakin ingin menghapus desa {{ $desa->nama_desa }}?', btnText: 'Hapus', btnColor: 'red', formId: 'form-delete-desa-{{ $desa->id }}' })" 
