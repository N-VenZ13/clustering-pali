                ...
                <div class="flex items-center gap-4">
                    ...
                    @if(Auth::id() !== $user->id)
                        <form id="form-delete-user-{{ $user->id }}" action="{{ route('users.destroy', $user->id) }}" method="POST" class="inline">
                            @csrf @method('DELETE')
                            <button type="button" 
                                    @click="$dispatch('open-confirm', { title: 'Hapus User?', msg: 'Apakah Anda yakin ingin menghapus akun {{ $user->name }} secara permanen?', btnText: 'Ya, Hapus', btnColor: 'red', formId: 'form-delete-user-{{ $user->id }}' })" 
                                    class="text-slate-400 hover:text-red-500" title="Hapus User">
