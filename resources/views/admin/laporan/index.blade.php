...
@else
<!-- KERTAS A4 (Area Cetak / Laporan Eksekutif) -->
<div id="print-area" class="max-w-[850px] mx-auto bg-white p-12 shadow-2xl border border-gray-300 min-h-[1122px] relative">
    ...
    <div class="text-center mb-8 relative z-10">
        <h3 class="text-lg font-extrabold text-[#1E293B] underline underline-offset-4 uppercase">Laporan Eksekutif Pemetaan Kesejahteraan Sosial</h3>
        <p class="text-sm font-semibold text-slate-600 mt-2">Tahun Data: {{ $tahun_aktif }}</p>
    </div>

    <!-- TEKS PENGANTAR EKSEKUTIF -->
    <div class="mb-8 text-justify text-sm text-[#1E293B] leading-relaxed relative z-10">
        <p class="mb-3">Dokumen ini menyajikan ringkasan kondisi kesejahteraan sosial wilayah perdesaan di Kabupaten Penukal Abab Lematang Ilir (PALI). Pengelompokan wilayah dilakukan secara objektif oleh sistem cerdas berdasarkan 8 (delapan) indikator ketersediaan infrastruktur dan fasilitas layanan dasar masyarakat.</p>
    ...
    <table class="w-full border-collapse border border-slate-300 mb-10 relative z-10 text-sm">
        <thead>
            <tr class="bg-slate-100 text-[#1E293B]">
                <th class="border border-slate-300 px-3 py-3 text-center w-10">No</th>
                <th class="border border-slate-300 px-3 py-3 text-left w-1/4">Kecamatan</th>
                <th class="border border-slate-300 px-3 py-3 text-center w-1/5">Status Wilayah</th>
                <th class="border border-slate-300 px-4 py-3 text-left">Ringkasan & Evaluasi Kondisi</th>
            </tr>
        </thead>
        <tbody>
            ...
            if($klaster == 1) $s++; elseif($klaster == 2) $b++; elseif($klaster == 3) $p++;
            if($klaster) $total++;
            }
            ...
            if($kec->status_akhir == 'Sejahtera') {
            $insight = "Kondisi wilayah sangat baik. Didominasi oleh $s desa sejahtera ($persen%). ";
            if($p > 0) $insight .= "Namun Pemda masih perlu memperhatikan $p desa yang tertinggal di wilayah ini.";
            } elseif($kec->status_akhir == 'Berkembang') {
            $insight = "Wilayah dalam fase transisi pembangunan. Terdapat $b desa berkembang. ";
            if($p > 0) $insight .= "Fokus intervensi infrastruktur harus diprioritaskan pada $p desa yang masih berstatus Perlu Perhatian.";
            else $insight .= "Tidak ada desa yang masuk kategori sangat tertinggal di kecamatan ini.";
            } else {
            $insight = "Wilayah ini memerlukan <b>Intervensi Prioritas</b>! Mayoritas wilayah ($p dari $total desa) sangat minim fasilitas dasar. Bantuan sosial dan fisik harus difokuskan ke kecamatan ini.";
            }
            }
            @endphp
            <tr>
                <td class="border border-slate-300 px-3 py-3 text-center align-top">{{ $index + 1 }}</td>
                <td class="border border-slate-300 px-3 py-3 align-top font-bold text-[#1E293B]">
                    {{ $kec->nama_kecamatan }}
                    <div class="text-[10px] text-slate-500 font-normal mt-1 border-t pt-1">
                        Total: {{ $total }} Desa<br>
                        S: {{ $s }} | B: {{ $b }} | P: {{ $p }}
                    </div>
                </td>
    ...
    <!-- LAMPIRAN: TABEL RINCIAN PER DESA -->
    <div class="relative z-10 break-before-page">
        <h3 class="text-sm font-bold text-[#1E293B] mb-3 uppercase border-b border-black pb-1">Lampiran: Status Detail Tingkat Desa</h3>
        <div class="grid grid-cols-2 gap-x-8 gap-y-6">
            @foreach($kecamatans as $kec)
            <div>
                <h4 class="font-bold text-[#1E3A8A] text-xs mb-2 bg-slate-100 p-1.5 border-l-4 border-[#1E3A8A]">{{ $kec->nama_kecamatan }}</h4>
                <table class="w-full text-[11px] border-collapse border border-gray-200">
                    <tbody>
                        ...
                        <tr class="border-b border-gray-100">
                            ...
                            <td class="py-1.5 px-2 border-l border-gray-100 font-semibold text-right {{ $warna }}">{{ $label }}</td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="2" class="py-1.5 px-2 text-slate-400 italic">Data kosong</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
